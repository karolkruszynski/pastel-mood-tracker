...
export const MoodByWeekday = ({ entries }: MoodByWeekdayProps) => {
  ...
  const getWeekdayStats = () => {
    ...
    const bestDay = bestDayKey !== "" ? 
      weekdays.find(day => day.key === bestDayKey) : null;
            ...
            {bestDay && (
              <div className="bg-secondary/50 p-4 rounded-lg">
                <p className="text-center font-medium">
                  ...
                  <span className="font-bold text-primary">{bestDay.name}s</span>!{" "}
                  <span className="text-sm text-muted-foreground">
                    (Average mood: {bestDay.avgMood} - {getMoodLabel(bestDay.avgMood)})
                  </span>
                </p>
              </div>
            )}
