...
export const MoodChart = ({ entries, days = 7, title }: MoodChartProps) => {
  const data = useMemo(() => {
    ...
    const startDate = entries[entries.length - 1]?.timestamp;
          ...
          <p>
            {mood !== null ? (
              <div className="flex items-center gap-2">
                {renderMoodIcon(mood)}
                <span>
                  {getMoodLabel(Math.round(mood))} ({mood.toFixed(1)})
                </span>
              </div>
            ) : (
              "No data"
  ...
  const renderMoodIcon = (mood: number | null) => {
    ...
    if (mood < 1.5)
      return <FrownIcon size={16} className="text-mood-terrible" />;
  ...
  const CustomDot = (props: any) => {
    ...
    if (
      lastDataPoint &&
      payload.date.getTime() === lastDataPoint.date.getTime() &&
      payload.averageMood !== null
    ) {
